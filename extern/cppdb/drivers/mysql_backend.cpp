...
column_type field_type(MYSQL_FIELD* field)
{
	if ( !field )
	{
		return null_type;
...
namespace unprep {
	...
	public:
		...
		virtual column_type type(int col)
		{
			if(col < 0 || col >=cols_)
				throw invalid_column();
			if(!res_)
				throw empty_row_access();
			return field_type(mysql_fetch_field_direct(res_, col));
		}
...
namespace prep {
	...
	public:
		...
		virtual column_type type(int col)
		{
			...
			if(!meta_)
				throw empty_row_access();
			return field_type(mysql_fetch_field_direct(meta_, col));
		}
