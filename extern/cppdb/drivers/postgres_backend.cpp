...
namespace cppdb {
	namespace postgresql {
		...
		public:
			...
			virtual column_type type(int col)
			{
				...
				case 26:   // oid
					return int_type;
				case 17:   // BYTEAOID
					return cppdb::blob_type;
				default:
					throw pqerror(res_, "unknown column type");
				}
			}
		private:
			...
			{
				check(col);
				return PQgetisnull(res_,current_,col) != 0;
			}
			PGresult *res_;
			PGconn *conn_;
			int rows_;
		...
		public:
			...
			virtual void* impl() const { return conn_; }
		private:
			PGconn *conn_;
