...
public:
	...
	virtual column_type type(int c)
	{
		if ( c < 0 || c >= int(types_.size()) )
		{
			throw invalid_column();
		}
		switch (types_[c])
	...
	result(rows_type &rows,std::vector<std::string> &names, std::vector<int> &types, int cols)
		: cols_(cols)
	{
		names_.swap(names);
		types_.swap(types);
...
public:
	...
	virtual result *query()
	{
		...
		return new result(rows,names,types,cols);
	}
