...
public:
	...
	path_list directories() const
	{
		boost::unique_lock<boost::mutex> lock(watch_descriptors_mutex_);
		path_list result;
		result.reserve(watch_descriptors_.size());
		for (watch_descriptors_t::const_iterator it = watch_descriptors_.begin(), end = watch_descriptors_.end(); it != end; ++it)
		{
...
private:
	void end_read(const boost::system::error_code &ec, std::size_t bytes_transferred)
	{
		if (!ec)
		{
			...
			while (pending_read_buffer_.size() > sizeof(inotify_event))
			{
				...
				pushback_event(dir_monitor_event(get_path(iev->wd, iev->name), type));
	...
	boost::filesystem::path get_path(int wd, std::string const& name)
	{
		boost::unique_lock<boost::mutex> lock(watch_descriptors_mutex_);
		watch_descriptors_t::left_map::iterator it = watch_descriptors_.left.find(wd);
		return it != watch_descriptors_.left.end()? it->second / name : name;
	}
	...
	mutable boost::mutex watch_descriptors_mutex_;
