...
struct multipart_form_parser
{
	explicit multipart_form_parser(pion::http::request_ptr request)
	{
		std::string const& content_type = request->get_header("Content-type");
		std::string const multipart = "multipart/form-data";
		std::string boundary = param(content_type, "boundary");
		if ( !boost::algorithm::iequals(content_type.substr(0, multipart.size()), multipart) || boundary.empty() )
		{
		...
		enum { BOUNDARY, HEADERS, CONTENT } state = BOUNDARY;

		char const* ptr = request->get_content();
		char const* end = ptr + request->get_content_length();
		assert(*end == 0 && "content buffer is null terminted");
		...
		while ( ptr < end )
		{
			...
			{
			case BOUNDARY:
				if ( (ptr + boundary.size() <= end) && memcmp(boundary.data(), ptr, boundary.size()) == 0 )
				{
					ptr += boundary.size();
					if ( check_crlf(ptr, end) )
					{
						state = HEADERS;
						continue;
					}
					else if ( ptr + 2 <= end && memcmp(boundary.data(), ptr, 2) == 0 )
					{
						return;
					}
				}
			...
			case HEADERS:
				...
				else
				{
					...
					char const* name_end = strchr(ptr, ':');
					if ( !name_end )
					{
						// throw invalid format
						return;
					}
					name_value.first.assign(ptr, name_end);
					ptr = name_end + 1;

					char const* value_end = strchr(ptr, '\r');
					if ( !value_end )
					{
						// throw invalid format
						return;
					}
					name_value.second.assign(ptr, value_end);
					ptr = value_end;
					...
					p.headers.insert(name_value);
				}
				break;
			case CONTENT:
				{
					p.data = ptr;
					char const* data_end = strstr(ptr, boundary.c_str());
					if ( !data_end )
					{
						// throw invalid format
						return;
					}
					data_end -= 2; // CRLF before bounadary
					ptr = data_end;
					if ( !check_crlf(ptr, end) )
					{
						// throw invalid format
					...
					p.size = data_end - p.data;
					if ( !p.size ) p.data = NULL;
					parts.push_back(p);
					p.reset();
					state = BOUNDARY;
				}
	...
	struct part
	{
		...
		std::string param(std::string const& header, std::string name) const
		{
			pion::ihash_multimap::const_iterator it = headers.find(header);
			return it != headers.end()? multipart_form_parser::param(it->second, name) : "";
		}
	};

	static bool check_crlf(char const*& ptr, char const* end)
	{
		static char const CRLF[2] = { '\r', '\n' };

		if ( (ptr + sizeof(CRLF) <= end) && memcmp(CRLF, ptr, sizeof(CRLF)) == 0 )
		{
			ptr += sizeof(CRLF);
			return true;
		}
	...
	/// Get value of parameter named param in a string str, empty string for absent
	static std::string param(std::string const& str, std::string name, bool unquote = true)
	{
		name += '=';
		std::string::size_type val_start = str.find(name);
		if ( val_start == std::string::npos )
		{
			return "";
		}
		val_start += name.length();

		std::string::size_type const val_end = str.find(';', val_start);
		std::string result = str.substr(val_start, val_end - val_start);
		if ( unquote && result.size() >= 2 && result[0] == '"' && result[result.size() - 1] == '"' )
		{
...
# if defined(BOOST_FILESYSTEM_VERSION) && BOOST_FILESYSTEM_VERSION >= 3
	...
	else if (name == "upload_directory") {
		m_upload_directory = value;
        // make sure that the directory exists
        if (! boost::filesystem::exists(m_upload_directory) || ! boost::filesystem::is_directory(m_upload_directory)) {
# if defined(BOOST_FILESYSTEM_VERSION) && BOOST_FILESYSTEM_VERSION >= 3
            const std::string dir_name = m_upload_directory.string();
#else
            const std::string dir_name = m_upload_directory.directory_string();
#endif
            BOOST_THROW_EXCEPTION( error::directory_not_found() << error::errinfo_dir_name(dir_name) );
		}
    } else {
...
# if defined(BOOST_FILESYSTEM_VERSION) && BOOST_FILESYSTEM_VERSION >= 3
               ...
               || http_request_ptr->get_method() == http::types::REQUEST_METHOD_DELETE)
    {
        ...
        if (!m_writable) {
			...
			boost::filesystem::path const& upload_dir = m_upload_directory.empty()? m_directory : m_upload_directory;
			...
			if (http_request_ptr->get_method() == http::types::REQUEST_METHOD_POST ||
				http_request_ptr->get_method() == http::types::REQUEST_METHOD_PUT)
			{
				multipart_form_parser mp_parser(http_request_ptr);

				std::vector<std::string> created_files;

				for (size_t i = 0, count = mp_parser.parts.size(); i != count; ++i)
				{
					multipart_form_parser::part const& part = mp_parser.parts[i];
					boost::filesystem::path filename = part.param("Content-Disposition", "filename");
					if ( filename.empty() )
					{
						continue;
					}
					...
					{
						created_files.push_back(filename.filename().string());
					}

					std::ios_base::openmode mode =
						http_request_ptr->get_method() == http::types::REQUEST_METHOD_POST? std::ios::app : std::ios::out;
					boost::filesystem::ofstream file_stream(filename, mode | std::ios::binary);
					file_stream.write(part.data, part.size);
					file_stream.close();
					if ( !boost::filesystem::exists(filename) )
					{
				...
				else
				{
					...
					writer->get_response().add_header(http::types::HEADER_LOCATION, created_files.front());

					writer->write_no_copy(CREATED_HTML_START);
					for (size_t i = 0, count = created_files.size(); i != count; ++i)
					{
						writer << created_files[i] << '\n';
					}
			...
			{
				boost::filesystem::path const filename = upload_dir / relative_path;
				if ( boost::filesystem::exists(filename) )
				{
...
void FileService::clear_cache(void)
{
	PION_LOG_DEBUG(m_logger, "Clearing cache for resource (" << get_resource() << ')');
	// clear cached files (if started again, it will re-scan)
	boost::mutex::scoped_lock cache_lock(m_cache_mutex);
	m_cache_map.clear();
}
